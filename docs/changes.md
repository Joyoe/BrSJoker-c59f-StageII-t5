...
- [JokerHide] Update package and process name validation logic
- [JokerHide] Some changes that prevents zygote deadlock
...
- [JokerInit] Detect 2SI after mounting `system_root` on legacy SAR devices
...
- [JokerInit] Support kernel cmdline `androidboot.fstab_suffix`
- [JokerInit] Support kernel initialized dm-verity on legacy SAR
...
- [JokerHide] Support hiding apps installed in secondary users (e.g. work profile)
- [JokerHide] Make zygote detection more robust
...
- [JokerInit] Rewritten 2SI support from scratch
- [JokerInit] Support when no `/sbin` folder exists (Android 11)
- [JokerInit] Dump fstab from device-tree to rootfs and force `init` to use it for 2SI devices
- [JokerInit] Strip out AVB for 2SI as it may cause bootloop
...
- [MagiskPolicy] Make policy statement closer to stock `*.te` format. Please check updated documentation or `jokerpolicy --help` for more details.
- [MagiskBoot] Support compressed `extra` blobs
- [MagiskBoot] Pad boot images to original size with zeros
- [JokerHide] Manipulate additional vendor properties

### v20.4

- [JokerInit] Fix potential bootloop in A-only 2SI devices
- [JokerInit] Properly support Tegra partition naming
...
- [JokerHide] Disable JokerHide by default
- [JokerHide] Add more potential detectable system properties
- [JokerHide] Add workaround for Xiaomi devices bootloop when JokerHide is enabled on cross region ROMs
...
- [JokerHide] Scan zygote multiple times
- [JokerInit] Support recovery images without /sbin/recovery binary. This will fix some A/B devices unable to boot to recovery after flashing Magisk
...
- [JokerInit] [2SI] Support non A/B setup (Android 10)
- [JokerHide] Fix bug that reject process names with ":"
- [MagicMount] Fix a bug that cause /product mirror not created

### v19.4

- [JokerInit] [SAR] Boot system-as-root devices with system mounted as /
- [JokerInit] [2SI] Support 2-stage-init for A/B devices (Pixel 3 Android 10)
- [JokerInit] [initramfs] Delay sbin overlay creation to post-fs-data
- [JokerInit] [SARCompat] Old system-as-root implementation is deprecated, no more future changes
- [JokerInit] Add overlay.d support for root directory overlay for new system-as-root implementation
- [MagiskSU] Unblock all signals in root shells (fix bash on Android)
- [MagicMount] Support replacing files in /product
- [JokerHide] Support Android 10's Zygote blastula pool
- [JokerHide] All random strings now also have random length
...
- [JokerHide] Hugely improve process monitor implementation, hopefully should no longer cause 100% CPU and daemon crashes
- [JokerInit] Wait for partitions to be ready for early mount, should fix bootloops on a handful of devices
- [JokerInit] Support EROFS used in EMUI 9.1
...
- [JokerInit] Add Kirin hi6250 support
...
- [JokerInit] New sbin overlay setup process for better compatibility
- [JokerInit] Allow long pressing volume up to boot to recovery in recovery mode
...
- [JokerHide] Use Zygote ptracing for monitoring new processes
- [JokerHide] Targets are now per-application component
- [JokerInit] Support Android Q (no logical partition support yet!)
- [MagiskPolicy] Support Android Q new split sepolicy setup
- [JokerInit] Move sbin overlay creation from main daemon post-fs-data to early-init
- [General] Service scripts now run in parallel
- [JokerInit] Directly inject magisk services to init.rc
...
- [JokerHide] Use component names instead of process names as targets
- [JokerHide] Add procfs protection on SDK 24+ (Nougat)
- [JokerHide] Remove the folder /.backup to prevent detection
- [JokerHide] Hide list is now stored in database instead of raw textfile in images
- [JokerHide] Add "--status" option to CLI
- [JokerHide] Stop unmounting non-custom related mount points
...
- [Daemon] Prevent logcat failures when /system/bin is magic mounting, could cause JokerHide to fail
...
- [JokerInit] Randomize Magisk service names to prevent detection (e.g. FGO)
...
- [JokerHide] Kill all processes with same UID of the target to workaround OOS embryo optimization
- [JokerInit] Move all sepolicy patches pre-init to prevent Pixel 2 (XL) boot service breakdown

### v16.7

- [Scripts] Fix boot image patching errors on Android P (workaround the strengthened seccomp)
- [JokerHide] Support hardlink based ns proc mnt (old kernel support)
- [Daemon] Fix permission of /dev/null after logcat commands, fix ADB on EMUI
- [Daemon] Log fatal errors only on debug builds
- [JokerInit] Detect early mount partname from fstab in device tree
...
- [Daemon] Check whether a valid Magisk Manager is installed on boot, if not, install stub APK embedded in jokerinit
...
- [JokerInit] Support detecting block names in upper case (Samsung)
- [MagiskBoot] Check DTB headers to prevent false detections within kernel binary
- [JokerHide] Compare mount namespace with PPID to make sure the namespace is actually separated, fix root loss
...
- [Daemon] Directly check logcat command instead of detecting logd, should fix logging and JokerHide on several Samsung devices
...
- [JokerInit] Remove `jokerinit_daemon`, the actual magisk daemon (magiskd) shall handle everything itself
...
- [joyoeprop] Add Protobuf encode/decode to support manipulating persist properties on Android P
- [JokerHide] Include app sub-services as hiding targets. This might significantly increase the amount of apps that could be properly hidden
...
- [JokerHide] Change property state to disable if logd is disabled
...
- [joyoeprop] Support Android P new property context files
- [MagiskPolicy] Add new rules for Android P

### v16.0

- [JokerInit] Support non `skip_initramfs` devices with slot suffix (Huawei Treble)
...
- [JokerInit] Prevent OnePlus angela `sepolicy_debug` from loading
- [JokerInit] Obfuscate Magisk socket entry to prevent detection and security
...
- [MagiskPolicy] Add new rules for proper Samsung support, should fix JokerHide
- [JokerInit] Support non `skip_initramfs` devices using split sepolicies (e.g. Zenfone 4 Oreo)
...
- [Daemon] New invincible implementation: use `jokerinit_daemon` to monitor sockets
...
- [JokerInit][MagiskBoot] Support ramdisk high compression mode
...
- [JokerHide] Add temporary /magisk removal
- [JokerHide] All changes above contributes to hiding from nasty apps like FGO and several banking apps
- [Magiskinit] Use jokerinit for all devices (dynamic initramfs)
...
- [joyoeprop] Add new "-p" flag, used to toggle whether alter/access the actual persist storage for persist props
...
- [JokerInit] Introduce a new init binary to support `skip_initramfs` devices (Pixel family)
...
- [joyoeprop] Fix a bug which delete props won't remove persist props not in memory
...
- [JokerHide] magisk.img mounted is no longer a requirement
...
- [JokerHide] Update to bypass Google CTS (2017.7.17)
- [joyoeprop] Properly support removing persist props
- [uninstaller] Remove Magisk Manager and persist props

### v13.2

- [jokerpolicy] Fix jokerpolicy segfault on old Android versions, should fix tons of older devices that couldn't use v13.1
- [JokerHide] Set proper selinux context while re-linking /sbin to hide Magisk, should potentially fix many issues
- [MagiskBoot] Change lzma compression encoder flag from `LZMA_CHECK_CRC64` to `LZMA_CHECK_CRC32`, kernel only supports latter
- [General] Core-only mode now properly mounts systemless hosts and jokerhide

### v13.1

- [General] Merge MagiskSU, jokerhide, joyoeprop, jokerpolicy into one binary
...
- [MagiskBoot] Moved boot patch logic into jokerboot binary
...
- [joyoeprop] Updated to latest AOSP upstream, support props from 5.0 to Android O
- [joyoeprop] Renamed all functions to prevent calling functions from external libc
- [jokerpolicy] Updated libsepol from official SELinux repo
- [jokerpolicy] Added xperm patching support (in order to make Android O work properly)
- [jokerpolicy] Updated rules for Android O, and Liveboot support
- [JokerHide] Remove pseudo permissive mode, directly hide permissive status instead
- [JokerHide] Remove unreliable list file monitor, change to daemon request mode
- [JokerHide] JokerHide is now enabled by default
- [JokerHide] Update unmount policies, passes CTS in SafetyNet!
- [JokerHide] Add more props for hiding
- [JokerHide] Remove background jokerhide daemon, spawn short life process for unmounting purpose
...
- [JokerHide] Support other logcat `am_proc_start` patterns
- [JokerHide] Change /sys/fs/selinux/enforce(policy) permissions if required
...
- [JokerHide] Add built in prop rules to fake KNOX status
Samsung apps requiring KNOX status to be 0x0 should now work (Samsung Pay not tested)
- [JokerHide] Remove all ro.build props, since they cause more issues than they benefit...
...
- [jokerpolicy] Rename sepolicy-inject to jokerpolicy
- [jokerpolicy] Update a rule to allow chcon everything properly
- [JokerHide] Prevent multirom crashes
- [JokerHide] Add patches for ro.debuggable, ro.secure, ro.build.type, ro.build.tags, ro.build.selinux
- [JokerHide] Change /sys/fs/selinux/enforce, /sys/fs/selinux/policy permissions for Samsung compatibility
...
- [jokerhide] Start JokerHide with scripts
...
- [joyoeprop] Support reading prop files that contains spaces in prop values
...
- [Addition] Busybox, JokerHide, hosts settings can now be applied instantly; no reboots required
...
- [joyoeprop] New tool "joyoeprop" is added to Magisk to replace most post-fs modules' functionality
- [joyoeprop] Magisk will now patch "ro.boot.verifiedbootstate", "ro.boot.flash.locked", "ro.boot.veritymode" to bypass Safety Net
