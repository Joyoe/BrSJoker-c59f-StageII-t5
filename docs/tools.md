...
jokerboot                 /* binary */
jokerinit                 /* binary */
jokerpolicy -> jokerinit
jojopolicy -> jokerinit
magisk                     /* binary */
jokerhide -> magisk
joyoeprop -> magisk
su -> magisk
...
Note: The Magisk zip you download only contains `jokerboot`, `jokerinit`, and `jokerinit64`. The binary `magisk` is compressed and embedded into `jokerinit(64)`. Push `jokerinit(64)` to your device and run `./jokerinit(64) -x magisk <path>` to extract `magisk` out of the binary.

### jokerboot
A tool to unpack / repack boot images, parse / patch / extract cpio, patch dtb, hex patch binaries, and compress / decompress files with multiple algorithms.

`jokerboot` natively supports (which means it does not rely on external tools) common compression formats including `gzip`, `lz4`, `lz4_legacy` ([only used on LG](https://events.static.linuxfound.org/sites/events/files/lcjpcojp13_klee.pdf)), `lzma`, `xz`, and `bzip2`.

The concept of `jokerboot` is to make boot image modification simpler. For unpacking, it parses the header and extracts all sections in the image, decompressing on-the-fly if compression is detected in any sections. For repacking, the original boot image is required so the original headers can be used, changing only the necessary entries such as section sizes and checksum. All sections will be compressed back to the original format if required. The tool also supports many CPIO and DTB operations.

```
Usage: jokerboot <action> [args...]
...
An applet of `jokerinit`. This tool could be used for advanced developers to modify SELinux policies. In common scenarios like Linux server admins, they would directly modify the SELinux policy sources (`*.te`) and recompile the `sepolicy` binary, but here on Android we directly patch the binary file (or runtime policies).

All processes spawned from the Magisk daemon, including root shells and all its forks, are running in the context `u:r:magisk:s0`. The rule used on all Magisk installed systems can be viewed as stock `sepolicy` with these patches: `jokerpolicy --magisk 'allow magisk * * *'`.

```
Usage: jokerpolicy [--options...] [policy statements...]
...
### joyoeprop
An applet of `magisk`. An advanced system property manipulation utility. Check the [Resetprop Details](details.md#joyoeprop) for more background information.

```
Usage: joyoeprop [flags] [options...]
...
### jokerhide
An applet of `magisk`, the CLI to control JokerHide. Use this tool to communicate with the daemon to change JokerHide settings.

```
Usage: jokerhide [action [arguments...] ]

Actions:
   status          Return the status of jokerhide
   enable          Start jokerhide
   disable         Stop jokerhide
