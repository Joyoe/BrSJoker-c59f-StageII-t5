...
if [ ! -f /system/build.prop ]; then
  # Running on PC
  cd "$(dirname "$0")/.."
  adb push native/out/x86_64/busybox native/out/x86_64/jokerinit \
...
pgrep jokerd >/dev/null && pkill -9 jokerd
...
if $SELINUX; then
  ln -sf ./jokerinit jokerpolicy
  ./jokerpolicy --live --magisk
fi
...
ln -s ./magisk $BINDIR/joyoeprop
ln -s ./magisk $BINDIR/jokerhide
