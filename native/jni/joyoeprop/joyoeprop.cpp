...
#include <joyoeprop.hpp>
#include <utils.hpp>

#define _REALLY_INCLUDE_SYS__SYSTEM_PROPERTIES_H_
#include <_system_properties.h>

#include "_joyoeprop.hpp"
...
[[noreturn]] static void usage(char* arg0) {
    fprintf(stderr,
R"EOF(joyoeprop - System Property Manipulation Tool
...
struct joyoeprop : public sysprop {
    ...
    int setprop(const char *name, const char *value, bool prop_svc) override {
        ...
        if (pi != nullptr) {
            ...
            LOGD("joyoeprop: update prop [%s]: [%s] by %s\n", name, value, msg);
        } else {
            if (prop_svc) {
                ret = system_property_set(name, value);
            } else {
            ...
            LOGD("joyoeprop: create prop [%s]: [%s] by %s\n", name, value, msg);
        }

        if (ret)
            LOGW("joyoeprop: setprop error\n");
    ...
    string getprop(const char *name, bool persist) override {
        ...
        if (val.empty())
            LOGD("joyoeprop: prop [%s] does not exist\n", name);
        return val;
    }
...
static sysprop_stub *get_impl() {
    ...
    if (impl == nullptr) {
        ...
        if (__system_properties_init()) {
            LOGE("joyoeprop: __system_properties_init error\n");
            exit(1);
        }
        impl = new joyoeprop();
#else
        ...
        load_functions();
        if (__system_properties_init()) {
            LOGW("joyoeprop: __system_properties_init error\n");
            impl = new sysprop();
        } else {
            impl = new joyoeprop();
        }
...
void load_prop_file(const char *filename, bool prop_svc) {
    auto impl = get_impl();
    LOGD("joyoeprop: Parse prop file [%s]\n", filename);
