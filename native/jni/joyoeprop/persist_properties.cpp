...
static void pb_getprop(prop_cb *prop_cb) {
    LOGD("joyoeprop: decode with protobuf [" PERSISTENT_PROPERTY_DIR "/persistent_properties]\n");
...
static bool file_getprop(const char *name, char *value) {
    ...
    if (fd < 0)
        return false;
    LOGD("joyoeprop: read prop from [%s]\n", path);
...
string persist_getprop(const char *name) {
    if (use_pb) {
        ...
        pb_getprop(&prop);
        if (prop.value[0]) {
            LOGD("joyoeprop: getprop (persist) [%s]: [%s]\n", name, prop.value);
            return prop.value;
        }
    } else {
    ...
    if (use_pb) {
        ...
        for (auto it = list.begin(); it != list.end(); ++it) {
            if (it->first == name) {
                ...
                LOGD("joyoeprop: encode with protobuf [" PERSISTENT_PROPERTY_DIR
                             "/persistent_properties.tmp]\n");
