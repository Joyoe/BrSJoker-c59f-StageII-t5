...
bool JokerInit::patch_sepolicy(const char *file) {
...
void SARBase::patch_rootdir() {
    ...
    // Extract magisk
    {
        auto magisk = mmap_data::ro("joker32.xz");
        unlink("joker32.xz");
        int fd = xopen("joker32", O_WRONLY | O_CREAT, 0755);
        unxz(fd, magisk.buf, magisk.sz);
        close(fd);
        patch_socket_name("joker32");
        if (access("joker64.xz", F_OK) == 0) {
            magisk = mmap_data::ro("joker64.xz");
            unlink("joker64.xz");
            fd = xopen("joker64", O_WRONLY | O_CREAT, 0755);
            unxz(fd, magisk.buf, magisk.sz);
            close(fd);
            patch_socket_name("joker64");
            xsymlink("./joker64", "magisk");
        } else {
            xsymlink("./joker32", "magisk");
        }
    }
...
void MagiskProxy::start() {
    ...
    auto magisk = mmap_data::ro("/sbin/joker32.xz");
    auto joker64 = mmap_data::ro("/sbin/joker64.xz");
    char custom_rules_dir[64];
    custom_rules_dir[0] = '\0';
    xreadlink(TMP_RULESDIR, custom_rules_dir, sizeof(custom_rules_dir));

    unlink("/sbin/magisk");
    unlink("/sbin/joker32.xz");
    unlink("/sbin/joker64.xz");
    rm_rf("/.backup");

    setup_tmp("/sbin");

    // Extract magisk
    int fd = xopen("/sbin/joker32", O_WRONLY | O_CREAT, 0755);
    unxz(fd, magisk.buf, magisk.sz);
    close(fd);
    patch_socket_name("/sbin/joker32");
    if (joker64.sz) {
        fd = xopen("/sbin/joker64", O_WRONLY | O_CREAT, 0755);
        unxz(fd, joker64.buf, joker64.sz);
        close(fd);
        patch_socket_name("/sbin/joker64");
        xsymlink("./joker64", "/sbin/magisk");
    } else {
        xsymlink("./joker32", "/sbin/magisk");
    }
