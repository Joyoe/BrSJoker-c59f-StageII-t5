...
class JokerInit : public BaseInit {
protected:
    mmap_data self;
    mmap_data config;
...
public:
    JokerInit(char *argv[], cmdline *cmd) : BaseInit(argv, cmd) {}
};

class SARBase : public JokerInit {
protected:
...
public:
    SARBase(char *argv[], cmdline *cmd) : JokerInit(argv, cmd) {}
};
...
class RootFSInit : public JokerInit {
private:
    void early_mount();
    void patch_rootfs();
public:
    RootFSInit(char *argv[], cmdline *cmd) : JokerInit(argv, cmd) {
        LOGD("%s\n", __FUNCTION__);
    }
    void start() override {
        early_mount();
...
class MagiskProxy : public JokerInit {
public:
    explicit MagiskProxy(char *argv[]) : JokerInit(argv, nullptr) {
        LOGD("%s\n", __FUNCTION__);
    }
    void start() override;
};
