...
class SuperuserViewModel(
    ...
    fun updatePolicy(policy: JoJoPolicy, isLogging: Boolean) = viewModelScope.launch {
        db.update(policy)
        val res = when {
            isLogging -> when {
    ...
    fun togglePolicy(item: PolicyRvItem, enable: Boolean) {
        fun updateState() {
            ...
            val policy = if (enable) JoJoPolicy.ALLOW else JoJoPolicy.DENY
            val app = item.item.copy(policy = policy)

            viewModelScope.launch {
                db.update(app)
                val res = if (app.policy == JoJoPolicy.ALLOW) R.string.su_snack_grant
