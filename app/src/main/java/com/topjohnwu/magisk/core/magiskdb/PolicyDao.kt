...
class PolicyDao : BaseDao() {
    ...
    suspend fun update(policy: JoJoPolicy) = buildQuery<Replace> {
        values(policy.toMap())
    }.commit()

    suspend fun <R: Any> fetchAll(mapper: (JoJoPolicy) -> R) = buildQuery<Select> {
        condition {
    ...
    private fun Map<String, String>.toPolicyOrNull(): JoJoPolicy? {
