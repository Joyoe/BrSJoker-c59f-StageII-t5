...
abstract class MagiskInstallImpl protected constructor(
    ...
    private fun extractFiles(): Boolean {
        ...
        try {
            ...
            if (isRunningAsStub) {
                ...
                // Also extract joker32 on non 64-bit only 64-bit devices
                val is32lib = Const.CPU_ABI_32?.let {
                    { entry: ZipEntry -> entry.name == "lib/$it/libjoker32.so" }
                } ?: { false }
    ...
    private fun processTar(input: InputStream, output: OutputStream): OutputStream {
        ...
        if (Config.recovery && recovery.exists() && boot.exists()) {
            ...
            arrayOf(
                "cd $installDir",
                "chmod -R 755 .",
                "./jokerboot unpack boot.img",
                "./jokerboot repack boot.img",
    ...
    private fun patchBoot(): Boolean {
        ...
        val job = shell.newJob().add("./jokerboot cleanup", "cd /")
