...
default_targets = ['magisk', 'jokerinit', 'jokerboot', 'busybox']
support_targets = default_targets + ['jokerpolicy', 'joyoeprop', 'test']
...
def dump_bin_headers():
    ...
    if not op.exists(stub):
        error('Build stub APK before building "jokerinit"')
...
def build_binary(args):
    ...
    if 'jokerinit' in args.target:
        dump_bin_headers()
        flag += ' B_INIT=1'

    if 'jokerpolicy' in args.target:
        flag += ' B_POLICY=1'

    if 'joyoeprop' in args.target:
        flag += ' B_PROP=1'

    if 'jokerboot' in args.target:
        flag += ' B_BOOT=1'
